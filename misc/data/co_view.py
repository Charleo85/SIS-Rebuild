...
line_list = data.map(lambda line: line.split(","))
pairs = line_list.map(lambda pair: (pair[0], pair[1]))
clean_pairs = pairs.distinct() # filter any duplicated tuples

# step 2: group all courses that a user has viewed into a list
# key = user_id, value = list of courses they viewed
view_list = clean_pairs.groupByKey()
...
# step 4: discard the user id, and count each co-view one time
# key = co-view of two courses, value = 1 (one occurrence)
occurrence = coview.map(lambda pair: (pair[1], 1))
...
# prints some info right after the output so we can use "grep"
